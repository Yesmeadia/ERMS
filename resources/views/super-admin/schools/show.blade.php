...
<div class="flex items-center gap-3 mb-6">
    ...
    <div class="flex-1">
        ...
        <p class="text-sm text-slate-400 mt-0.5">{{ $school->code }} · {{ $school->state }}, {{ $school->zone }}</p>
    </div>
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-6">
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
                ...
                <div><p class="text-slate-500">Exam Centre Designation</p>
                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium mt-0.5 {{ $school->is_centre ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20' : 'bg-slate-800 text-slate-400 border border-slate-700/60' }}">
                        <span class="w-1.5 h-1.5 rounded-full {{ $school->is_centre ? 'bg-indigo-400' : 'bg-slate-500' }}"></span>
                        {{ $school->is_centre ? 'Designated Exam Centre' : 'Standard School' }}
                    </span>
                </div>
                <div class="col-span-2"><p class="text-slate-500">Address</p><p class="text-slate-200 font-medium mt-0.5">{{ $school->address }}, {{ $school->state }}, {{ $school->zone }}</p></div>
            </div>
        </div>
