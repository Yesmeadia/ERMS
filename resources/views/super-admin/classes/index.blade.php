...
@section('page_description', 'Manage all class definitions available for registration')
@section('content')
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-slate-800/60">
                    <th class="text-left px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">Class Name
                    </th>
            ...
            <tbody class="divide-y divide-slate-800/60">
                @forelse($classes as $class)
                    <tr class="hover:bg-slate-800/20 transition-colors">
                        ...
                        <td class="px-6 py-4 text-slate-200 font-semibold">₹{{ number_format($class->registration_fee, 2) }}
                        </td>
                        <td class="px-6 py-4 text-center">
                            @if($class->status)
                                <span
                                    class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"><span
                                        class="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>Active</span>
                            @else
                                <span
                                    class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-rose-500/10 text-rose-400 border border-rose-500/20"><span
                                        class="w-1.5 h-1.5 rounded-full bg-rose-400"></span>Inactive</span>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form method="POST" action="{{ route('admin.classes.toggle-status', $class) }}">
                                    @csrf
                                    <button type="submit"
                                        class="p-2 rounded-lg text-slate-400 hover:bg-slate-700 hover:text-amber-400 transition-all cursor-pointer"
                                        title="Toggle Status">
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-16 text-center text-slate-500">No classes found. <a
                                href="{{ route('admin.classes.create') }}" class="text-indigo-400">Add one →</a></td>
                    </tr>
                @endforelse
            </tbody>
        </table>
