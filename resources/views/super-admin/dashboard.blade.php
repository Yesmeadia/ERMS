...
@section('content')
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
        @php
            $statCards = [
                ...
                [
                    ...
                    'value' => $stats['hall_tickets_issued'],
                    'icon' => null,
                    'paths' => [
        ...
        {{-- Online Exam: Total Created Exams --}}
        @if(isset($onlineExamStats))
            <a href="{{ \Illuminate\Support\Facades\Route::has('admin.online-exams.index') ? route('admin.online-exams.index') : url('/admin/online-exams') }}"
                class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-5 flex items-start gap-4 hover:border-indigo-500/50 hover:bg-slate-900/80 transition-all shadow-md group cursor-pointer block">
                <div
                    class="flex items-center justify-center w-12 h-12 rounded-xl border bg-indigo-600/10 border-indigo-500/20 text-indigo-400 group-hover:scale-105 transition-transform shrink-0">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-6 h-6">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center justify-between">
                        <p class="text-2xl font-bold text-white group-hover:text-indigo-300 transition-colors">
                            {{ number_format($onlineExamStats['total_exams'] ?? 0) }}</p>
                        <svg xmlns="http://www.w3.org/2000/svg"
                            class="w-4 h-4 text-slate-500 group-hover:text-indigo-400 group-hover:translate-x-0.5 transition-all"
                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                        </svg>
                    </div>
                    <p class="text-xs text-slate-400 mt-0.5">Total Created Exams</p>

                    <div class="flex items-center gap-2 mt-3 pt-2.5 border-t border-slate-800/60 text-xs flex-wrap">
                        <span class="inline-flex items-center gap-1.5 text-emerald-400 font-medium">
                            <span
                                class="w-2 h-2 rounded-full bg-emerald-400 {{ ($onlineExamStats['live_exams'] ?? 0) > 0 ? 'animate-pulse' : '' }}"></span>
                            Live: <strong
                                class="text-emerald-400 font-semibold font-mono">{{ $onlineExamStats['live_exams'] ?? 0 }}</strong>
                        </span>
                        <span class="text-slate-600">&bull;</span>
                        <span class="inline-flex items-center gap-1.5 text-slate-400 font-medium">
                            <span class="w-2 h-2 rounded-full bg-slate-500"></span>
                            Drafted: <strong
                                class="text-slate-200 font-semibold font-mono">{{ $onlineExamStats['draft_exams'] ?? 0 }}</strong>
                        </span>
                    </div>
                </div>
            </a>

            {{-- Online Exam: Participating Students --}}
            <div
                class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-5 flex items-start gap-4 hover:border-slate-700/60 transition-all shadow-md">
                <div
                    class="flex items-center justify-center w-12 h-12 rounded-xl border bg-purple-600/10 border-purple-500/20 text-purple-400 shrink-0">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z" />
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-2xl font-bold text-white">{{ number_format($onlineExamStats['total_students'] ?? 0) }}</p>
                    <p class="text-xs text-slate-400 mt-0.5">Participating Students</p>

                    <div class="flex items-center gap-2 mt-3 pt-2.5 border-t border-slate-800/60 text-xs flex-wrap">
                        <span class="inline-flex items-center gap-1.5 text-amber-400 font-medium">
                            <span
                                class="w-2 h-2 rounded-full bg-amber-400 {{ ($onlineExamStats['live_students'] ?? 0) > 0 ? 'animate-pulse' : '' }}"></span>
                            Live: <strong
                                class="text-amber-400 font-semibold font-mono">{{ $onlineExamStats['live_students'] ?? 0 }}</strong>
                        </span>
                        <span class="text-slate-600">&bull;</span>
                        <span class="inline-flex items-center gap-1.5 text-rose-400 font-medium">
                            <span class="w-2 h-2 rounded-full bg-rose-500"></span>
                            Terminated: <strong
                                class="text-rose-400 font-semibold font-mono">{{ $onlineExamStats['terminated_students'] ?? 0 }}</strong>
                        </span>
                        <span class="text-slate-600">&bull;</span>
                        <span class="inline-flex items-center gap-1.5 text-indigo-400 font-medium">
                            <span class="w-2 h-2 rounded-full bg-indigo-400"></span>
                            Completed: <strong
                                class="text-indigo-400 font-semibold font-mono">{{ $onlineExamStats['completed_students'] ?? 0 }}</strong>
                        </span>
                    </div>
                </div>
            </div>
    ...
    <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-6 shadow-md">
        ...
        @forelse($recentActivities as $activity)
            <div class="flex items-start gap-3 py-3 border-b border-slate-800/60 last:border-0">
                <div
                    class="w-8 h-8 rounded-full bg-indigo-600/10 border border-indigo-500/20 text-indigo-400 flex items-center justify-center shrink-0 text-xs font-bold">
                    {{ mb_substr($activity->causer?->name ?? 'S', 0, 1) }}
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-sm text-slate-300 truncate">{{ $activity->description }}</p>
                    <p class="text-xs text-slate-500 mt-0.5">{{ $activity->causer?->name ?? 'System' }} ·
