    ...
    <form method="GET" action="{{ route('admin.reports.index') }}" id="reportFilterForm">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            <div>
                ...
                <select name="type" id="reportTypeSelect" onchange="updateReportFilters(this.value)" class="w-full px-4 py-2.5 rounded-xl bg-slate-800/60 border border-slate-700/60 text-slate-200 text-sm focus:outline-none focus:border-indigo-500/50">
                    <optgroup label="Registration Reports">
                        <option value="school_wise" {{ $reportType === 'school_wise' ? 'selected' : '' }}>School-wise Registration</option>
                        <option value="zone_wise" {{ $reportType === 'zone_wise' ? 'selected' : '' }}>Zone-wise Registration</option>
            ...
            {{-- Zone Filter --}}
            <div id="filterZone" class="hidden">
                <label class="block text-xs font-medium text-slate-400 mb-1.5">Zone</label>
                <select name="zone" class="w-full px-4 py-2.5 rounded-xl bg-slate-800/60 border border-slate-700/60 text-slate-200 text-sm focus:outline-none focus:border-indigo-500/50">
            ...
            {{-- Class Filter --}}
            <div id="filterClass" class="hidden">
            ...
            {{-- Attendance: School Filter --}}
            <div id="filterSchool" class="hidden">
                <label class="block text-xs font-medium text-slate-400 mb-1.5">Assigned School</label>
            ...
            <div id="filterAttendanceCategory" class="hidden">
                <label class="block text-xs font-medium text-slate-400 mb-1.5">Category</label>
                <select name="category_id" class="w-full px-4 py-2.5 rounded-xl bg-slate-800/60 border border-slate-700/60 text-slate-200 text-sm focus:outline-none focus:border-indigo-500/50">
                    <option value="">All Categories</option>
            ...
            <div id="filterDate" class="hidden">
                <label class="block text-xs font-medium text-slate-400 mb-1.5">Date</label>
                <input type="date" name="date" value="{{ request('date', '2026-08-30') }}" class="w-full px-4 py-2.5 rounded-xl bg-slate-800/60 border border-slate-700/60 text-slate-200 text-sm focus:outline-none focus:border-indigo-500/50">
            </div>
...
<div class="flex flex-wrap gap-3 mb-6">
    <a href="{{ route('admin.reports.export', array_filter(['type' => $reportType, 'examination_id' => $examinationId, 'class_id' => $classId, 'category_id' => $categoryId, 'zone' => $zone, 'school_id' => request('school_id'), 'date' => request('date'), 'format' => 'excel'])) }}"
    ...
    <a href="{{ route('admin.reports.export', array_filter(['type' => $reportType, 'examination_id' => $examinationId, 'class_id' => $classId, 'category_id' => $categoryId, 'zone' => $zone, 'school_id' => request('school_id'), 'date' => request('date'), 'format' => 'csv'])) }}"
    ...
    <a href="{{ route('admin.reports.export', array_filter(['type' => $reportType, 'examination_id' => $examinationId, 'class_id' => $classId, 'category_id' => $categoryId, 'zone' => $zone, 'school_id' => request('school_id'), 'date' => request('date'), 'format' => 'pdf'])) }}"
...
<div class="mb-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        @foreach([
            'zone_wise' => 'Zone-wise Registration',
...
@push('scripts')
    <script @nonce>
        // Filter visibility logic — run on page load and on report type change
        function updateReportFilters(type) {
            const filterMap = {
                zone_wise:           ['filterZone', 'filterClass', 'filterCategory'],
                school_wise:         ['filterZone'],
                class_wise:          ['filterZone', 'filterCategory'],
                category_wise:       ['filterZone', 'filterClass'],
                examination_wise:    [],
                approved:            [],
                rejected:            [],
                hall_ticket:         [],
                attendance:          ['filterSchool', 'filterAttendanceCategory', 'filterDate'],
                attendance_category: [],
                attendance_gender:   [],
            };

            const allFilters = ['filterZone', 'filterClass', 'filterCategory', 'filterSchool', 'filterAttendanceCategory', 'filterDate'];
            allFilters.forEach(id => {
                const el = document.getElementById(id);
                if (el) el.classList.add('hidden');
            });

            (filterMap[type] || []).forEach(id => {
                const el = document.getElementById(id);
        ...
        document.addEventListener('DOMContentLoaded', function () {
            // Apply filter visibility based on current server-rendered report type
            updateReportFilters(document.getElementById('reportTypeSelect').value);
            ...
            const renderChart = (chartElement, chartData) => {
                ...
                const baseOptions = {
                    chart: { background: 'transparent', toolbar: { show: false }, type: chartType, height: 300, stacked: !!chartData.stacked },
                    theme: { mode: 'dark' },
                ...
                if (chartType === 'donut' || chartType === 'pie') {
                    ...
                    chartOptions = {
                        ...baseOptions,
                        series: chartData.series,
                        xaxis: { categories: chartData.categories ?? [], labels: { style: { colors: '#64748b', fontSize: '11px' } } },
                        stroke: { width: 2 },
                        fill: { opacity: 0.15 },
                        legend: { show: true, position: 'bottom', labels: { colors: '#94a3b8' } },
                    };
