...
<div class="space-y-6">
    {{-- Header --}}
    <div>
        <h2 class="text-xl font-bold text-white font-outfit">Examination Centres</h2>
        <p class="text-sm text-slate-400 mt-0.5">Designate schools as examination venues and allocate candidates to centres before issuing hall tickets.</p>
    </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-6 space-y-4">
                ...
                <form method="GET" class="flex gap-3">
                    <input type="text" name="school_search" value="{{ request('school_search') }}" placeholder="Search schools to designate..."
                    ...
                    <button type="submit" class="bg-slate-750 hover:bg-slate-700 text-white text-xs font-semibold px-4 py-2 rounded-xl transition-all cursor-pointer">Filter</button>
                    @if(request('school_search'))
                        <a href="{{ route('admin.exam-centres.index') }}" class="bg-slate-800 hover:bg-slate-750 text-slate-400 text-xs font-semibold px-4 py-2 rounded-xl transition-all flex items-center justify-center">Clear</a>
                    @endif
                </form>
                ...
                <div class="overflow-hidden border border-slate-800/80 rounded-xl">
                    <table class="w-full text-xs text-left">
                        ...
                        <tbody class="divide-y divide-slate-800/40 text-slate-350">
                            @forelse($allSchools as $school)
                                <tr class="hover:bg-slate-800/20 transition-colors">
                                    <td class="py-3.5 px-4">
                                        ...
                                        <p class="text-[10px] text-slate-500 mt-0.5">{{ $school->code }} · {{ $school->email }}</p>
                                    </td>
                                    ...
                                    <td class="py-3.5 px-4">
                                        <div class="flex items-center justify-end">
                                            <form method="POST" action="{{ route('admin.exam-centres.toggle', $school) }}">
                                                @csrf
                                                @if($school->is_centre)
                                                    <button type="submit" class="inline-flex items-center gap-1.5 bg-indigo-500/15 hover:bg-rose-500/10 hover:text-rose-400 hover:border-rose-500/20 text-indigo-400 border border-indigo-500/25 px-2.5 py-1.5 rounded-lg font-bold transition-all cursor-pointer">
                                                        <span class="w-1.5 h-1.5 rounded-full bg-indigo-400"></span> Exam Centre
                                                    </button>
                                                @else
                                                    <button type="submit" class="inline-flex items-center gap-1.5 bg-slate-800 hover:bg-slate-700 text-slate-400 border border-slate-700/60 px-2.5 py-1.5 rounded-lg font-semibold transition-all cursor-pointer">
                                                        Set Exam Centre
                                                    </button>
                                                @endif
                                            </form>
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="py-8 px-4 text-center text-slate-500 italic">No schools matched the filter parameters.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-6 space-y-4">
                <div class="flex items-center justify-between border-b border-slate-800/60 pb-3">
                    ...
                    <span class="text-xs text-indigo-400 font-bold uppercase tracking-wider font-mono bg-indigo-500/10 border border-indigo-500/20 px-2 py-0.5 rounded">Active Centres</span>
                </div>

                <form method="GET" class="flex gap-3">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search active venues by name, zone..."
                           class="bg-slate-800/50 border border-slate-700/60 rounded-xl px-4 py-2 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500 w-full sm:w-80">
                    <button type="submit" class="bg-slate-750 hover:bg-slate-700 text-white text-xs font-semibold px-4 py-2 rounded-xl transition-all cursor-pointer">Search</button>
                    @if(request('search'))
                        <a href="{{ route('admin.exam-centres.index') }}" class="bg-slate-800 hover:bg-slate-750 text-slate-400 text-xs font-semibold px-4 py-2 rounded-xl transition-all flex items-center justify-center font-semibold">Clear</a>
                    @endif
                </form>
                        ...
                        <tbody class="divide-y divide-slate-800/40 text-slate-350">
                            @forelse($centres as $centre)
                                <tr class="hover:bg-slate-800/20 transition-colors">
                                    ...
                                    <td class="py-3.5 px-4">
                                        <div class="flex items-center justify-end">
                                            <form method="POST" action="{{ route('admin.exam-centres.toggle', $centre) }}" onsubmit="return confirm('Are you sure you want to revoke Exam Centre status? This will preserve existing students but they won\'t assign to this school in new bulk allocations.')">
                                                @csrf
                                                <button type="submit" class="p-2 bg-slate-800 hover:bg-rose-500/10 hover:text-rose-400 text-slate-400 rounded-lg border border-slate-750 transition-all cursor-pointer" title="Revoke Centre Designation">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="py-8 px-4 text-center text-slate-550 italic">No designated Examination Centres found. Use the directory list above to designate schools.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="space-y-6">
            <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-6 space-y-5">
                <div class="border-b border-slate-800/60 pb-3">
                    ...
                    <p class="text-[11px] text-slate-400 mt-1">Assign an Exam Centre venue to all candidates registered under a specific school-examination filter group.</p>
                </div>

                <form method="POST" action="{{ route('admin.exam-centres.assign') }}" class="space-y-4">
                    @csrf
                    <div>
                        <label for="school_id" class="block text-xs font-semibold text-slate-400 mb-1.5">Registered School</label>
                    ...
                    <div>
                        <label for="examination_id" class="block text-xs font-semibold text-slate-400 mb-1.5">Examination Session</label>
                    ...
                    <div>
                        <label for="centre_id" class="block text-xs font-semibold text-slate-400 mb-1.5">Target Venue (Exam Centre)</label>
                    ...
                    <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-semibold py-3 rounded-xl transition-all shadow-md shadow-indigo-600/10 text-xs cursor-pointer">
