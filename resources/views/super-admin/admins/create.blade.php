@extends('layouts.app')
@section('page_title', 'Add Board Admin')
@section('content')
<div class="max-w-3xl">
    {{-- Header --}}
    <div class="mb-6">
    ...
    <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-6 lg:p-8">
        <form method="POST" action="{{ route('admin.admins.store') }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="pb-4 border-b border-slate-800/40">
                ...
                <div class="flex items-center gap-6" x-data="{ imgPreview: '' }">
                    <div class="w-20 h-20 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center overflow-hidden shrink-0 shadow-inner">
                        ...
                        <template x-if="!imgPreview">
                            <div class="w-full h-full bg-gradient-to-tr from-indigo-600 to-indigo-400 text-white flex items-center justify-center font-bold text-xl uppercase text-center p-2">
                                Admin
                            </div>
                        </template>
                    </div>
