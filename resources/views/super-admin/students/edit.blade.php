...
@section('page_title', 'Edit Student Registration (Super Admin)')
...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.students.update', $student) }}" enctype="multipart/form-data"
        ...
        @photo-crop-done.document="if ($event.detail.hiddenId === 'photographCroppedAdminEdit') { photoPreview = $event.detail.base64; photoMissingError = false; }"
        @submit="
            if (!photoPreview && !$refs.photoInput.files.length) {
                $event.preventDefault();
                photoMissingError = true;
                $nextTick(() => {
                    const el = document.getElementById('adminEditPhotoPreview');
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-6 space-y-4">
                ...
                <div>
                    <label class="block text-xs font-medium text-slate-400 mb-1.5">Student Name <span
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-slate-400 mb-1.5">Gender <span
                                class="text-rose-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-slate-400 mb-1.5">Date of Birth <span
                                class="text-rose-500">*</span></label>
                ...
                <div>
                    <label class="block text-xs font-medium text-slate-400 mb-1.5">Father's Name <span
                            class="text-rose-500">*</span></label>
                ...
                <div>
                    <label class="block text-xs font-medium text-slate-400 mb-1.5">Mother's Name <span
                            class="text-rose-500">*</span></label>
                ...
                <div>
                    <label class="block text-xs font-medium text-slate-400 mb-1.5">Mobile Number <span
                            class="text-rose-500">*</span></label>
                    <input type="tel" name="mobile_number" value="{{ old('mobile_number', $student->mobile_number) }}"
            ...
            {{-- Academic & Registration Settings Card --}}
            <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-6 space-y-4">
                <div class="border-b border-slate-800/60 pb-3 flex items-center gap-2 text-indigo-400">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                        stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M4.26 10.147a60.436 60.436 0 00-.491 6.347A48.62 48.62 0 0112 20.904a48.62 48.62 0 018.231-4.41 60.46 60.46 0 00-.49-6.347m-15.482 0a48.69 48.69 0 0115.482 0m-15.482 0L12 14.197 19.74 10.15m-16.5 3.03h.008v.008H3.24v-.008zm16.5 0h.008v.008h-.008v-.008z" />
                    </svg>
                    <h3 class="text-sm font-bold uppercase tracking-wider text-slate-200">Registration & Academic Details
                    </h3>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-slate-400 mb-1.5">Class <span
                                class="text-rose-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-slate-400 mb-1.5">Category <span
                                class="text-rose-500">*</span></label>
                ...
                <div>
                    <label class="block text-xs font-medium text-slate-400 mb-1.5">Examination Session <span
                            class="text-rose-500">*</span></label>
                    <select name="examination_id" required
                        class="w-full bg-slate-800/40 border border-slate-700/60 rounded-xl px-4 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-indigo-500">
                        <option value="">Select Examination Session</option>
                        @foreach($examinations as $exam)
                            <option value="{{ $exam->id }}" @selected(old('examination_id', $student->examination_id) == $exam->id)>{{ $exam->name }} ({{ $exam->status }})</option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="registration_number"
                            ...
                            class="w-full bg-slate-800/40 border border-slate-700/60 rounded-xl px-4 py-2.5 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500">
                        @error('registration_number') <p class="text-xs text-rose-400 mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-slate-400 mb-1.5">Payment Status <span
                                class="text-rose-500">*</span></label>
                        <select name="payment_status" required
                            class="w-full bg-slate-800/40 border border-slate-700/60 rounded-xl px-4 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-indigo-500">
                            @foreach($paymentStatuses as $ps)
                                <option value="{{ $ps }}" @selected(old('payment_status', $student->payment_status) === $ps)>
                                    {{ $ps }}</option>
                            @endforeach
                        </select>
                        @error('payment_status') <p class="text-xs text-rose-400 mt-1">{{ $message }}</p> @enderror
                    </div>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-slate-400 mb-1.5">Verification Status <span
                                class="text-rose-500">*</span></label>
                        <select name="status" required
                            class="w-full bg-slate-800/40 border border-slate-700/60 rounded-xl px-4 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-indigo-500">
                            @foreach($statuses as $st)
                                <option value="{{ $st }}" @selected(old('status', $student->status) === $st)>{{ $st }}</option>
                            @endforeach
                        </select>
                ...
                <div>
                    <label class="block text-xs font-medium text-slate-400 mb-1.5">Rejection/Verification Remarks</label>
                    <textarea name="remarks" rows="2"
                        placeholder="Provide remarks if rejecting or reviewing the registration"
                        class="w-full bg-slate-800/40 border border-slate-700/60 rounded-xl p-3 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500">{{ old('remarks', $student->remarks) }}</textarea>
                    @error('remarks') <p class="text-xs text-rose-400 mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-4 mt-1.5">
                        ...
                        <div class="flex-1">
                            ...
                            <input type="hidden" id="photographCroppedAdminEdit" name="photograph_cropped" value="">
                            {{-- Actual file input — checks 3:4 ratio before launching cropper --}}
                            <input
                                type="file"
                                id="adminEditPhotoFileInput"
                                x-ref="photoInput"
                                name="photograph"
                                         ...
                                         reader.onload = (e) => {
                                             ...
                                             window.dispatchEvent(new CustomEvent('open-photo-cropper', {
                                                 detail: {
                                                     ...
                                                     targetPreviewId: 'adminEditPhotoPreview',
                                                     targetHiddenId: 'photographCroppedAdminEdit',
                                                     targetFileInputId: 'adminEditPhotoFileInput'
                                                 }
                                             }));
                                         };
