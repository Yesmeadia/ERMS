...
{{-- Back Link --}}
<div class="mb-6">
    <a href="{{ route('admin.students.index') }}" class="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-slate-100 transition-colors">
...
{{-- Form Container --}}
<form method="POST" action="{{ route('admin.students.update', $student) }}" enctype="multipart/form-data" class="space-y-6" x-data='{ photoPreview: @js($student->photo_url) }'>
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-6 space-y-4">
            <div class="border-b border-slate-800/60 pb-3 flex items-center gap-2 text-indigo-400">
                ...
                <h3 class="text-sm font-bold uppercase tracking-wider text-slate-200">Registration & Academic Details</h3>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="registration_number" value="{{ old('registration_number', $student->registration_number) }}" placeholder="Optional/Auto-generated"
                           class="w-full bg-slate-800/40 border border-slate-700/60 rounded-xl px-4 py-2.5 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500">
                    @error('registration_number') <p class="text-xs text-rose-400 mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-xs font-medium text-slate-400 mb-1.5">Payment Status <span class="text-rose-500">*</span></label>
                    <select name="payment_status" required class="w-full bg-slate-800/40 border border-slate-700/60 rounded-xl px-4 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-indigo-500">
                        @foreach($paymentStatuses as $ps)
                            <option value="{{ $ps }}" @selected(old('payment_status', $student->payment_status) === $ps)>{{ $ps }}</option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-medium text-slate-400 mb-1.5">Verification Status <span class="text-rose-500">*</span></label>
                    <select name="status" required class="w-full bg-slate-800/40 border border-slate-700/60 rounded-xl px-4 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-indigo-500">
            ...
            <div>
                ...
                <textarea name="remarks" rows="2" placeholder="Provide remarks if rejecting or reviewing the registration"
