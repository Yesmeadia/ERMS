    ...
    <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-5 mb-6">
        <form method="GET" action="{{ route('admin.students.index') }}"
            ...
            <div>
                ...
                <select name="examination_id" id="filter-exam"
            ...
            <div>
                ...
                <select name="school_id" id="filter-school"
                    class="w-full px-3 py-2.5 rounded-xl bg-slate-800/60 border border-slate-700/60 text-slate-200 text-sm focus:outline-none focus:border-indigo-500/50">
            ...
            <div class="xl:col-span-6 flex justify-end gap-3 mt-1">
                <a href="{{ route('admin.students.index') }}"
                    class="px-5 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm font-medium rounded-xl transition-colors">
                    Clear Filters
                </a>
                <button type="submit" id="apply-filters-btn"
                    class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded-xl transition-colors cursor-pointer">
                    Apply Filters
                </button>
            </div>
        </form>
            ...
            <table class="w-full text-sm text-left">
                <thead>
                    <tr class="border-b border-slate-800/60">
                        ...
                        <th class="px-5 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider text-center">
                            Gender</th>
                        <th class="px-5 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">Exam Centre</th>

                        <th class="px-5 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider text-center">
                            Status</th>
                        <th class="px-5 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider text-right">
                            Actions</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-slate-800/20 transition-colors">
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-9 h-9 rounded-lg overflow-hidden border border-slate-800 bg-slate-900 shrink-0">
                            ...
                            <td class="px-5 py-3.5">
                                @if($student->registration_number)
                                    <span
                                        class="font-mono text-xs font-bold text-indigo-300 bg-indigo-500/10 border border-indigo-500/20 px-2.5 py-1 rounded-lg">
                            ...
                            {{-- Exam Centre --}}
                            <td class="px-5 py-3.5 text-slate-400 text-xs max-w-[130px] truncate"
                                title="{{ $student->centre->name ?? '—' }}">
                                {{ $student->centre->name ?? '—' }}
                            </td>
                            ...
                            {{-- Status --}}
                            <td class="px-5 py-3.5 text-center text-xs text-slate-300">
                                {{ $displayStatus }}
                            </td>
                            ...
                            <td class="px-5 py-3.5 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    {{-- Edit Student Button --}}
                                    <a href="{{ route('admin.students.edit', $student->id) }}" id="edit-student-{{ $student->id }}"
                                        class="px-3 py-1.5 text-xs font-bold rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white shadow-md shadow-indigo-600/10 transition-colors flex items-center gap-1.5 cursor-pointer whitespace-nowrap"
                                        title="Edit Student">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-6 py-16 text-center">
                                ...
                                <a href="{{ route('admin.students.index') }}"
                                    class="text-indigo-400 hover:text-indigo-300 text-sm mt-2 inline-block">Clear all
                                    filters</a>
                            </td>
                        </tr>
                    @endforelse
