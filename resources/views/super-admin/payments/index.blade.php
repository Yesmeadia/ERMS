...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
    <div>
        <h2 class="text-xl font-bold text-white">Board Financial Reports</h2>
        <p class="text-sm text-slate-400 mt-0.5">Monitor school payouts, track collected fees, and view financial audits.</p>
    </div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-5 relative overflow-hidden group">
        ...
        <p class="text-[10px] text-slate-500 mt-4 font-semibold uppercase tracking-wider">Outstanding from unpaid drafts</p>
    </div>
    ...
    <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-5 relative overflow-hidden group">
        ...
        <p class="text-[10px] text-slate-500 mt-4 font-semibold uppercase tracking-wider">Schools with completed payments</p>
    </div>
</div>
...
<div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-5 mb-8">
    ...
    <form method="GET" action="{{ route('admin.payments.index') }}" class="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end">
        ...
        <div>
            ...
            <select name="school_id" class="w-full bg-slate-800/50 border border-slate-700/60 rounded-xl px-4 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-indigo-500">
                <option value="">All Schools</option>
                @foreach($schools as $school)
                    <option value="{{ $school->id }}" @selected(request('school_id') == $school->id)>{{ $school->name }} ({{ $school->code }})</option>
                @endforeach
            </select>
        </div>
        <div>
        ...
        <div class="flex gap-2 justify-end mt-2">
            ...
            @if(request()->hasAny(['search', 'school_id', 'status']))
                <a href="{{ route('admin.payments.index') }}" class="bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm font-medium px-5 py-2.5 rounded-xl transition-all flex items-center justify-center">Clear</a>
            @endif
        </div>
    </form>
</div>
        ...
        <table class="w-full text-sm text-left">
            <thead>
                <tr class="border-b border-slate-800/60">
                    ...
                    <th class="px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">School details</th>
                    <th class="px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">Transaction ID</th>
                    <th class="px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider text-center">Candidates</th>
                    <th class="px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider text-right">Amount (INR)</th>
                    <th class="px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider text-center">Status</th>
                    <th class="px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider text-right">Breakdown</th>
                </tr>
            </thead>
                ...
                @forelse($payments as $payment)
                    <tr class="hover:bg-slate-800/10 transition-colors" x-data="{ expanded: false }">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <p class="text-[10px] text-slate-500 uppercase tracking-wider font-semibold mt-0.5">Code: {{ $payment->school->code }}</p>
                        </td>
                        <td class="px-6 py-4 text-indigo-400 font-mono font-bold text-xs">{{ $payment->transaction_id }}</td>
                        <td class="px-6 py-4 text-center font-mono font-semibold text-slate-300">{{ $payment->students_count ?? $payment->students()->count() }}</td>
                        <td class="px-6 py-4 text-right font-bold text-slate-200 font-mono">₹{{ number_format($payment->amount, 2) }}</td>
                    ...
                    <tr x-show="expanded" style="display: none;" class="bg-slate-950/40">
                        <td colspan="7" class="px-8 py-4 border-l-4 border-indigo-500">
                            <h4 class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Candidates included in Transaction:</h4>
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 max-h-56 overflow-y-auto pr-2">
                                @foreach($payment->students as $student)
                                    <div class="flex items-center gap-2 bg-slate-900/60 border border-slate-800/40 p-2 rounded-xl text-xs">
