        ...
        <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-6 space-y-5 shadow-xl">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" name="marks_obtained" id="marks_obtained" min="0" placeholder="e.g. 35" class="w-full px-4 py-2.5 rounded-xl bg-slate-800/60 border border-slate-700/60 text-slate-200 text-sm focus:outline-none focus:border-indigo-500/50" required>
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-400 mb-1.5">Maximum Marks <span class="text-rose-400">*</span></label>
                    <input type="number" name="max_marks" id="max_marks" min="1" value="{{ old('max_marks', $defaultMaxMarks ?? 50) }}" placeholder="e.g. {{ $defaultMaxMarks ?? 50 }}" class="w-full px-4 py-2.5 rounded-xl bg-slate-800/60 border border-slate-700/60 text-slate-200 text-sm focus:outline-none focus:border-indigo-500/50" required>
                    <p class="text-[11px] text-slate-500 mt-1">Category ({{ $student->category?->name ?? 'Standard' }}): <strong>{{ $defaultMaxMarks ?? 50 }} max marks</strong></p>
                </div>
            ...
            <div>
                ...
                <textarea name="remarks" id="remarks" rows="2" placeholder="Enter manual remarks or comments..." class="w-full px-4 py-2.5 rounded-xl bg-slate-800/60 border border-slate-700/60 text-slate-200 text-sm focus:outline-none focus:border-indigo-500/50"></textarea>
            </div>
        </div>
