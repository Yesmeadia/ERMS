...
@section('page_title', 'Result Statement Generation Progress')

@section('content')
<div class="max-w-4xl mx-auto" x-data="batchProgress({
    batchId: {{ $batch->id }},
    statusUrl: '{{ route('admin.results.batches.status', $batch) }}',
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 text-xs text-slate-400 mb-1">
                <a href="{{ route('admin.results.index') }}" class="hover:text-indigo-400 transition-colors">Results Management</a>
                <span>/</span>
                <span class="text-slate-200">Export Batch #{{ $batch->id }}</span>
            </div>
            <h1 class="text-xl font-bold text-slate-100">{{ $batch->title }}</h1>
            <p class="text-xs text-slate-400 mt-1">
                Scope: <span class="text-slate-200 font-semibold">{{ $batch->subtitle ?? 'Consolidated Statement' }}</span> · 
                Session: <span class="text-indigo-400 font-semibold">{{ $batch->examination->name ?? ($batch->filter_criteria['labels']['examination'] ?? 'Examination') }}</span>
            </p>
        </div>
        <div>
            <a href="{{ route('admin.results.index') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white text-xs font-semibold rounded-xl border border-slate-700/60 transition-all">
    ...
    {{-- Main Progress Card --}}
    <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-6 mb-6 backdrop-blur-xl relative overflow-hidden shadow-xl">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
                <span class="text-xs font-bold uppercase tracking-wider text-slate-400">Generation Status</span>
                <div class="flex items-center gap-2.5 mt-1.5">
                    ...
                    <template x-if="status === 'pending'">
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-amber-500/10 text-amber-400 border border-amber-500/20">
                            <span class="w-2 h-2 rounded-full bg-amber-400 animate-ping"></span> Queued in Background
                        </span>
                    </template>
                    ...
                    <template x-if="status === 'completed_with_errors' || (status === 'completed' && failedParts > 0)">
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-amber-500/10 text-amber-400 border border-amber-500/20">
                            <span class="w-2 h-2 rounded-full bg-amber-400"></span> Partially Ready
                        </span>
                    </template>
                    ...
                    <span class="text-xs text-slate-400">
                        <span x-text="completedStudents"></span> of <span x-text="totalStudents"></span> candidates processed
                    </span>
                </div>
            </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-slate-800/60">
            <div class="bg-slate-800/40 rounded-xl p-3 border border-slate-800/40">
                <span class="text-[10px] uppercase font-bold text-slate-500 tracking-wider">Total Candidates</span>
    ...
    {{-- PDF Parts Section --}}
    <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-6 mb-6 shadow-xl">
        <div class="flex items-center justify-between mb-4">
            <div>
                <h3 class="text-sm font-bold text-slate-200 uppercase tracking-wider">Result Statement PDF Files</h3>
                <p class="text-xs text-slate-400 mt-0.5">PDFs are divided into optimized batches for fast, high-resolution printing.</p>
            </div>
            <template x-if="failedParts > 0">
                <form method="POST" action="{{ route('admin.results.batches.retry', $batch) }}">
                    @csrf
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="flex items-center gap-2">
                                <h4 class="text-sm font-semibold text-slate-200" x-text="`Result Statement - Part ${part.part_number}`"></h4>
                                <span class="text-xs text-slate-500" x-text="`(${part.total_students} Candidates)`"></span>
                            </div>
                            <p class="text-xs text-slate-400 mt-0.5">
                                <span x-show="part.status === 'completed'" class="text-emerald-400 font-medium">✓ Ready for instant download</span>
                                <span x-show="part.status === 'processing'" class="text-indigo-400 font-medium">⚡ Rendering PDF pages in background...</span>
                                <span x-show="part.status === 'pending'" class="text-slate-500">Queued in worker...</span>
    ...
    return {
        ...
        async checkStatus() {
            ...
            try {
                ...
                console.error('Result batch status poll error:', err);
            }
        },
