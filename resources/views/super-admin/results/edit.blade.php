...
@section('page_title', 'Edit Exam Result')
...
<div class="max-w-3xl">
    ...
    {{-- Result Form --}}
    <form method="POST" action="{{ route('admin.results.update', $result->id) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-400 mb-1.5">Grade</label>
                    <input type="text" name="grade" value="{{ $result->grade }}" placeholder="e.g. A+ (Leave blank to auto-calculate)" class="w-full px-4 py-2.5 rounded-xl bg-slate-800/60 border border-slate-700/60 text-slate-200 text-sm focus:outline-none focus:border-indigo-500/50">
                </div>
            </div>
