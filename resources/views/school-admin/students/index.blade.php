...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
    ...
    {{-- Bulk Excel Import --}}
    <div class="bg-slate-900/40 backdrop-blur-md border border-slate-800/60 rounded-2xl shadow-2xl overflow-hidden"
         x-data="{ expanded: false, excelName: '', dragging: false }">
        ...
        <div class="px-5 py-4 border-b border-slate-800/60 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-xs font-bold text-slate-200">Bulk Excel Import</h3>
                    <p class="text-[10px] text-slate-500 mt-0.5">Register multiple students at once</p>
                </div>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{ route('school.students.import.template') }}"
                   class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-800/60 hover:bg-slate-800 border border-slate-700/60 text-slate-300 hover:text-white rounded-lg text-[10px] font-semibold transition-all"
                   title="Download Excel Template">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-3 h-3 text-indigo-400">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
                    </svg>
                    Template
                </a>
                <button @click="expanded = !expanded"
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600/10 hover:bg-indigo-600/20 border border-indigo-500/20 text-indigo-400 rounded-lg text-[10px] font-semibold transition-all cursor-pointer">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-3 h-3 transition-transform duration-200" :class="expanded ? 'rotate-180' : ''">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                    </svg>
                    <span x-text="expanded ? 'Collapse' : 'Import'">Import</span>
                </button>
            </div>
        </div>
        ...
        <div class="px-5 py-4 flex items-center gap-0">
            <div class="flex items-center gap-2 shrink-0">
                <span class="w-5 h-5 rounded-full bg-indigo-500/20 border border-indigo-500/30 text-indigo-400 text-[9px] font-black flex items-center justify-center">1</span>
                <span class="text-[10px] text-slate-400 font-medium">Download template</span>
            </div>
            <div class="flex-1 h-px bg-slate-800 mx-3"></div>
            <div class="flex items-center gap-2 shrink-0">
                <span class="w-5 h-5 rounded-full bg-indigo-500/20 border border-indigo-500/30 text-indigo-400 text-[9px] font-black flex items-center justify-center">2</span>
                <span class="text-[10px] text-slate-400 font-medium">Fill student data</span>
            </div>
            <div class="flex-1 h-px bg-slate-800 mx-3"></div>
            <div class="flex items-center gap-2 shrink-0">
                <span class="w-5 h-5 rounded-full bg-indigo-500/20 border border-indigo-500/30 text-indigo-400 text-[9px] font-black flex items-center justify-center">3</span>
                <span class="text-[10px] text-slate-400 font-medium">Upload & import</span>
            </div>
        </div>
        ...
        <div x-show="expanded" x-transition:enter="transition ease-out duration-200"
             ...
             class="px-5 pb-5 border-t border-slate-800/60 pt-5" style="display:none;">

            @if($examinations->where('status', 'Registration Started')->isEmpty())
                <div class="flex items-center gap-3 p-4 bg-rose-950/20 border border-rose-900/30 text-rose-300 rounded-xl">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4 shrink-0 text-rose-400">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
                    </svg>
                    <p class="text-xs font-medium">No active exam sessions available for bulk import.</p>
                </div>
            @else
                <form method="POST" action="{{ route('school.students.import') }}" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 mb-1.5 uppercase tracking-wider">
                            Exam Session <span class="text-rose-500">*</span>
                        </label>
                        <select name="import_examination_id" required
                                class="w-full bg-slate-950/50 border border-slate-800/80 rounded-xl px-4 py-2.5 text-xs text-slate-200 focus:outline-none focus:border-indigo-500 transition-all font-medium">
                            <option value="">Select Examination Session</option>
                            @foreach($examinations->where('status', 'Registration Started') as $exam)
                    ...
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 mb-1.5 uppercase tracking-wider">
                            Excel / CSV File <span class="text-rose-500">*</span>
                        </label>
                        <input type="file" x-ref="excelInput" name="excel_file" required accept=".xlsx,.xls,.csv"
                               class="hidden"
                               @change="excelName = $event.target.files[0] ? $event.target.files[0].name : ''">
                        <div @click="$refs.excelInput.click()"
                             @dragover.prevent="dragging = true"
                             @dragleave.prevent="dragging = false"
                             @drop.prevent="dragging = false; const f = $event.dataTransfer.files[0]; if(f){ $refs.excelInput.files = $event.dataTransfer.files; excelName = f.name; }"
                             :class="dragging ? 'border-indigo-500 bg-indigo-500/5' : 'border-slate-700/60 bg-slate-950/30 hover:border-indigo-500/50 hover:bg-slate-950/50'"
                             class="relative flex flex-col items-center justify-center gap-2 border-2 border-dashed rounded-xl px-4 py-6 cursor-pointer transition-all duration-200 group">

                            {{-- Icon --}}
                            <div class="w-10 h-10 rounded-xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center group-hover:bg-indigo-500/15 transition-colors">
                            ...
                            <div class="text-center">
                                <p class="text-xs font-semibold text-slate-300 group-hover:text-white transition-colors"
                                   x-text="excelName || 'Click or drag & drop your file here'">
                                    Click or drag & drop your file here
                                </p>
                                <p class="text-[10px] text-slate-500 mt-0.5" x-show="!excelName">.xlsx, .xls, .csv accepted</p>
                                <p class="text-[10px] text-indigo-400 mt-0.5 font-medium" x-show="excelName">File ready to upload ✓</p>
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- Submit --}}
                    <button type="submit"
                            class="w-full bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold py-2.5 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/20 active:scale-[0.98] duration-150">
