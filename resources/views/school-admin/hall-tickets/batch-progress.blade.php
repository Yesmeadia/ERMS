...
<div class="max-w-4xl mx-auto" x-data="batchProgress({
    batchId: {{ $batch->id }},
    statusUrl: '{{ route('school.hall-tickets.batches.status', $batch) }}',
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 text-xs text-slate-400 mb-1">
                <a href="{{ route('school.hall-tickets.index') }}" class="hover:text-indigo-400 transition-colors">Hall Tickets</a>
                <span>/</span>
                <span class="text-slate-200">Batch #{{ $batch->id }}</span>
            </div>
            <h1 class="text-xl font-bold text-slate-100">Hall Ticket Bulk Generation</h1>
            <p class="text-xs text-slate-400 mt-1">
                Session: <span class="text-indigo-400 font-semibold">{{ $batch->examination->name ?? 'Examination' }}</span> · 
                School: <span class="text-slate-300 font-medium">{{ $batch->school->name ?? 'School' }}</span>
            </p>
        </div>
        <div>
            <a href="{{ route('school.hall-tickets.index') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white text-xs font-semibold rounded-xl border border-slate-700/60 transition-all">
    ...
    <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <template x-if="failedParts > 0">
                <form method="POST" action="{{ route('school.hall-tickets.batches.retry', $batch) }}">
                    @csrf
