...
<div class="max-w-4xl mx-auto" x-data="batchProgress({
    batchId: {{ $batch->id }},
    statusUrl: '{{ route('school.results.batches.status', $batch) }}',
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 text-xs text-slate-400 mb-1">
                <a href="{{ route('school.results.index') }}" class="hover:text-indigo-400 transition-colors">Results</a>
                <span>/</span>
                <span class="text-slate-200">Export Batch #{{ $batch->id }}</span>
            </div>
            <h1 class="text-xl font-bold text-slate-100">{{ $batch->title }}</h1>
            <p class="text-xs text-slate-400 mt-1">
                Institution: <span class="text-slate-200 font-semibold">{{ $batch->school->name ?? 'School' }}</span> · 
        ...
        <div>
            <a href="{{ route('school.results.index') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white text-xs font-semibold rounded-xl border border-slate-700/60 transition-all">
    ...
    <div class="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-6 mb-6 shadow-xl">
        <div class="flex items-center justify-between mb-4">
            <div>
                <h3 class="text-sm font-bold text-slate-200 uppercase tracking-wider">Institution Result Statement PDFs</h3>
                <p class="text-xs text-slate-400 mt-0.5">PDFs are divided into optimized parts for fast, clean printing.</p>
            </div>
            <template x-if="failedParts > 0">
                <form method="POST" action="{{ route('school.results.batches.retry', $batch) }}">
                    @csrf
